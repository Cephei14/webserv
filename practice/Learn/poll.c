...
void handle_client_data(int listener, int *fd_count, struct pollfd *pfds, int *pfd_i)
{
	...
	else // We got data from a client
	{
		...
		for(int j = 0; j < *fd_count; j++) // Send to everyone!
		{
			...
			if (dest_fd != listener && dest_fd != sender_fd) // Except the listener & ourselves
			{
...
void process_connections(int listener, int *fd_count, int *fd_size, struct pollfd **pfds)
{
	...
	{
		if ((*pfds)[i].revents & (POLLIN | POLLHUP))// Check if someone's ready to read
		{
			if ((*pfds)[i].fd == listener) // If it's the listener => a new connection
...
{
	int listener;	 // Listening socket descriptor
	int fd_size = 5; // Start off with room for 5 connections (We'll realloc as necessary)
	int fd_count = 0;
	struct pollfd *pfds = malloc(sizeof *pfds * fd_size);

	listener = get_listener_socket(); // Set up and get a listening socket
	if (listener == -1)
	{
		fprintf(stderr, "error getting listening socket\n");
		exit(1);
	}
	pfds[0].fd = listener; // Add the listener to set;
	...
	while(1)
	{
		...
		process_connections(listener, &fd_count, &fd_size, &pfds); // Run through connections looking for data to read
	}
	free(pfds);
}
