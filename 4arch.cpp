...
void HTTPResponse::handle_body(const std::string& path)
{
...
void HTTPResponse::construct_response(const HTTPRequest& req)
{
	...
	if (_reason.empty()) 
	{
		if (_statusCode == 200) _reason = "OK";
		else if (_statusCode == 404) _reason = "Not Found";
		else if (_statusCode == 501) _reason = "Not Implemented";
		else _reason = "Error";
	}
	...
	for(it = header_req.begin(); it != header_req.end(); it++)
	{
